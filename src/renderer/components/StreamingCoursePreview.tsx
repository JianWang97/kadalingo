...
const StreamingCoursePreview: React.FC<StreamingCoursePreviewProps> = ({
  ...
  const streamRef = useRef<AsyncGenerator<StreamChunk, void, unknown> | null>(
    null
  );
    ...
    switch (chunk.type) {
      ...
      case "sentence":
        if (
          ...
          "chinese" in chunk.data
        ) {
          const sentence = chunk.data as {
            chinese: string;
            english: string;
            phonetic?: string;
    ...
    for (let i = 0; i < generateConfig.lessonCount; i++) {
      ...
      for (
        ...
        lessonSentences.push({
          ...
          phonetic: generatedSentence.phonetic,
  ...
  const groupSentencesByLessons = () => {
    ...
    for (let i = 0; i < generateConfig.lessonCount; i++) {
      ...
      const endIndex = Math.min(
        startIndex + sentencesPerLesson,
        partialCourse.sentences.length
      );
      const lessonSentences = partialCourse.sentences.slice(
        startIndex,
        endIndex
      );
  ...
  return (
    <div
      ref={containerRef}
      className={`${
        isInDrawer ? "h-full" : "min-h-screen"
      } bg-gradient-to-br from-violet-50 via-purple-50 to-blue-50 overflow-y-auto scroll-smooth`}
    >
              ...
              <div className="flex items-center">
                ...
                {isGenerating && (
                  <div className="ml-2 flex items-center">
                    ...
                    <div
                      className="w-1 h-1 bg-blue-400 rounded-full animate-pulse mr-1"
          ...
          <div className="space-y-4">
            {groupSentencesByLessons().map((lesson) => (
              <div
                ...
                className="backdrop-blur-sm bg-white/90 rounded-xl shadow-lg border border-white/20"
              >
                ...
                <div className="p-4 space-y-3">
                  {lesson.sentences.map((sentence, index) => {
                    ...
                    const isLastSentence =
                      lesson.lessonNumber ===
                        groupSentencesByLessons().length &&
          ...
          <div className="sticky bottom-4 backdrop-blur-sm bg-white/90 rounded-xl shadow-lg border border-white/20 p-4">
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600">
                ...
                <span className="ml-2">
                  共 {partialCourse.sentences.length} 个句子，
                  {generateConfig.lessonCount} 个课时
                </span>
              </div>
              <button
